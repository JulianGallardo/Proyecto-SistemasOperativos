...
void sem_Espera(int* sem) {//Simula un sem_wait con pipes.
    char buffer;
    read(sem[0], &buffer, 1); // Espera hasta que se escriba en el pipe para simular la operacion sem_wait
}

void sem_Envia(int* sem) {//Simula un sem_post con pipes.
    char buffer = 'x';
    write(sem[1], &buffer, 1); // Escribe en el pipe para simular la operacion sem_post
}
...
void processA() {
    while(1) {
        ...
        printf("A");
        fflush(stdout);
        sem_Envia(semB); // Envía una señal para el proceso B
    }
}
...
void processB() {
    while (1) {
        ...
        printf("B");
        fflush(stdout);
        sem_Envia(semC); //Envia una señal para el proceso C
        sem_Envia(semA);
    }
}
